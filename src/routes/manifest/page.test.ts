...
vi.mock('$lib/metadata', () => ({
	...
	getManufacturingDate: () => 'MFG: 2005.05.10',
...
describe('Manifest Page - Projects Section', () => {
	...
	it('renders each project name as a link to GitHub', () => {
		...
		const links = container.querySelectorAll('section#projects a[target="_blank"]');
		expect(links.length).toBe(3);
		expect(links[0].textContent?.trim()).toBe('project-alpha');
		expect(links[0].getAttribute('href')).toBe('https://github.com/ItsThompson/project-alpha');
	...
	it('renders primary language with color dot', () => {
		...
		const dots = container.querySelectorAll('section#projects span.rounded-full');
		expect(dots.length).toBe(2); // Only 2 projects have primaryLanguage
		expect((dots[0] as HTMLElement).style.backgroundColor).toBe('rgb(53, 114, 165)');
	});
	...
	it('renders the MetadataBar with project count and source', () => {
		...
		const section = container.querySelector('section#projects');
		expect(section?.textContent).toContain('PINNED: 3 // SOURCE: github.com/ItsThompson');
	});
